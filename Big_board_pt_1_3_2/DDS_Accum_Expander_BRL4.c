...
volatile unsigned int DDS_phase_2 ;
...
volatile unsigned int ddc =(M_PI/5720);
volatile unsigned int DDS_increment = Fout*two32/Fs ; //42949673 ;
volatile unsigned int DDS_increment_2 = Fout_2*two32/Fs ; //42949673 ;
volatile unsigned int counter_swoop = 5721;
volatile unsigned int counter_chirp = 5721;
volatile unsigned int counter_silent = 5721;
...
{
    int junk;
    if(press == 1 && counter_swoop <= 5720){
        counter_swoop++;
        DDS_increment = (int)((-260*sin(-ddc*counter_swoop)+1740)*tfs);
        DDS_phase += DDS_increment ;
    }
    else if(press == 2 && counter_chirp <= 5720){
        counter_chirp++;
        DDS_increment_2 = (int)(1.53*0.0001*counter_chirp*counter_chirp+2000)*tfs;
        DDS_phase_2 += DDS_increment_2 ;

    }
    else if(press == 3 && counter_silent <= 5720){
        counter_silent++;
    }
    mT2ClearIntFlag();
    ...
    if(press==1){
        ...
        if (counter_swoop < (attack_time + decay_time + sustain_time)){
            current_amplitude = (counter_swoop <= attack_time)? 
                current_amplitude + attack_inc : 
                (counter_swoop <= attack_time + sustain_time)? current_amplitude:
        ...
        DAC_data_A = (int)(current_amplitude_2*sine_table[DDS_phase_2>>24]) + 2048 ; // for testing sine_table[DDS_phase>>24]

        // update amplitude envelope 
        if (counter_chirp < (attack_time + decay_time + sustain_time)){
            current_amplitude_2= (counter_chirp <= attack_time)? 
                current_amplitude_2 + attack_inc : 
                (counter_chirp <= attack_time + sustain_time)? current_amplitude_2:
                    current_amplitude_2 - decay_inc ;
        }
        else {
            current_amplitude_2 = 0 ;
        }
        DAC_data_B = (int) current_amplitude_2;
   ...
   //                         counter_chirp=0;
    ...
    //                    if(b_pressed[i]==3){
     //                       press=3;
      //                      counter_silent=0;
       //                 }
       //                 while(counter_swoop<= 5720 || counter_chirp<=5720 || counter_silent<=5720){};
        //                i++;
         //               
          //          }
           //     }
...
void main(void) {
 //SYSTEMConfigPerformance(PBCLK);
  ...
  while (1){
      ...
      porta = PORTReadBits(IOPORT_A,BIT_0);
      printf("%d", porta);
      printf("hw");
      if(porta==1){
          record=1;
